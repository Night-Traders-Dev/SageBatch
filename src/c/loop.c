...
static SageValue sage_fn_main_2();
static SageValue sage_fn_test_1();
...
static SageValue sage_fn_test_1() {
    ...
    sage_raise(sage_make_dict_from_entries(1, (const char*[]){"a"}, (SageValue[]){sage_number(1)}));
    return sage_gc_return(&sage_gc_frame, sage_nil());
}

static SageValue sage_fn_main_2() {
    SageSlot sage_local_e_4 = sage_slot_undefined();
    SageSlot sage_local_i_3 = sage_slot_undefined();
    SageSlot* sage_gc_roots[2] = {&sage_local_e_4, &sage_local_i_3};
    SageGcFrame sage_gc_frame;
    sage_gc_push_frame(&sage_gc_frame, sage_gc_roots, 2);
    sage_define_slot(&sage_local_i_3, sage_number(0));
    while (sage_truthy(SAGE_LT(sage_load_slot(&sage_local_i_3, "i"), sage_number(10000)))) {
        {
            ...
            if (setjmp(sage_try_stack[sage_try_depth - 1]) == 0) {
                ...
                _caught = 1;
                sage_define_slot(&sage_local_e_4, sage_exception_value);
            }
            sage_try_depth--;
            if (_caught) {
                (void)sage_assign_slot(&sage_local_i_3, "i", SAGE_ADD(sage_load_slot(&sage_local_i_3, "i"), sage_number(1)));
            }
        }
    }
    sage_print_ln(sage_string("Done"));
    return sage_gc_return(&sage_gc_frame, sage_nil());
}
