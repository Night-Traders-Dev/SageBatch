...
static SageValue sage_method_Foo_emit(SageValue _self, int _argc, SageValue* _argv);

static SageSlot sage_global_s_2;
static SageSlot sage_global_f_1;

void sage_gc_mark_program_globals(void) {
    if (sage_global_s_2.defined) sage_gc_mark_value(sage_global_s_2.value);
    if (sage_global_f_1.defined) sage_gc_mark_value(sage_global_f_1.value);
}


static SageValue sage_method_Foo_emit(SageValue _self, int _argc, SageValue* _argv) {
    SageSlot sage_local_b_5 = sage_slot_undefined();
    SageSlot sage_local_a_4 = sage_slot_undefined();
    SageSlot sage_local_self_3 = sage_slot_undefined();
    SageSlot* sage_gc_roots[3] = {&sage_local_b_5, &sage_local_a_4, &sage_local_self_3};
    SageGcFrame sage_gc_frame;
    sage_gc_push_frame(&sage_gc_frame, sage_gc_roots, 3);
    sage_define_slot(&sage_local_self_3, _self);
    sage_define_slot(&sage_local_a_4, _argv[0]);
    sage_define_slot(&sage_local_b_5, _argv[1]);
    (void)_argc;
...
int main(int argc, char** argv) {
    ...
    sage_global_s_2 = sage_slot_undefined();
    sage_global_f_1 = sage_slot_undefined();
    SageSlot* sage_gc_global_roots[2] = {&sage_global_s_2, &sage_global_f_1};
    SageGcFrame sage_gc_main_frame;
    sage_gc_push_frame(&sage_gc_main_frame, sage_gc_global_roots, 2);
    sage_register_class("Foo", NULL);
    sage_register_method("Foo", "emit", sage_method_Foo_emit);
    sage_define_slot(&sage_global_f_1, sage_construct("Foo", NULL, 0, NULL));
    sage_define_slot(&sage_global_s_2, sage_string("hello"));
    (void)sage_call_method(sage_load_slot(&sage_global_f_1, "f"), "emit", 2, (SageValue[]){sage_string("A"), sage_upper(sage_load_slot(&sage_global_s_2, "s"))});
